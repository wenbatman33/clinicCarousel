$(function () {
  ...
  function getRoomData() {
    // $.get("./assets/json/data_01.json", {}, function (res) {
  ...
  function getData() {
    // $.get("./assets/json/data_02.json", {}, function (
  ...
  function showRoomData(res) {
    ...
    for (j = 0; j < roomDataLength; j++) {
      ...
      if (
        dayjs(currentTime).isAfter(dayjs(sTime)) &&
        dayjs(currentTime).isBefore(dayjs(eTime))
      ) {
  ...
  function loop(num) {
    ...
    for (i = num; i < len; i++) {
      if (patientList[i]) {
        ...
        element +=
          '<span class="name">' + patientList[i].PT_PATNAME + "</span>";
        element += "</li>";
      }
    }
    $(".patientUL").html(element);
